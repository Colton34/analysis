...
var studentInfo = [
    {
        kaohao: '1', name: '阿友', className: '一班', score_ts: 150, gradeRank_ts: 1, classRank_ts: 1, score_123456: 88, gradeRank_123456: 2, classRank_123456: 3
    }, {
        kaohao: '2', name: '阿友2', className: '一班', score_ts: 150, gradeRank_ts: 1, classRank_ts: 1, score_123456: 88, gradeRank_123456: 2, classRank_123456: 3
    }, {
        
        kaohao: '3', name: '阿友3', className: '一班', score_ts: 150, gradeRank_ts: 1, classRank_ts: 1, score_123456: 88, gradeRank_123456: 2, classRank_123456: 3
    }, {
        kaohao: '4', name: '阿友4', className: '一班', score_ts: 150, gradeRank_ts: 1, classRank_ts: 1, score_123456: 88, gradeRank_123456: 2, classRank_123456: 3
    }, {
        kaohao: '5', name: '阿友5', className: '一班', score_ts: 150, gradeRank_ts: 1, classRank_ts: 1, score_123456: 88, gradeRank_123456: 2, classRank_123456: 3
    }
]


var headerMapper = {
    kaohao: '考号', name: '姓名', class: '班级', totalScore: '总分', groupRank: '排名', classRank: '班级排名', ts: '总分', score: '分数'
}
...
class RankReportTableView extends React.Component {
    constructor(props) {
        ...
        // todo: headerMapper作为类的属性之一;
        _.forEach(this.props.examInfo.papers, paperObj => {
            headerMapper[paperObj.pid] = paperObj.name;
        })
        var headSelect = {};
        ...
        if (searchStr === '') {
            if (this.state.currentPaper.name === '全科') {
                ...
                _.forEach(this.props.rankCache, (classGroup, scoreType) => {
                    var classGroups = _.pick(this.props.rankCache[scoreType], this.state.currentClasses)
                    _.forEach(classGroup, (students, className) => {
                        _.forEach(students, student => {
                            kaohaoMap[student.kaohao] = true;
                        })
            ...
            _.forEach(filteringData, (studentInfo) => {
                for (var key in studentInfo) {
                    if(studentInfo[key].indexOf(searchStr) !== -1) {
                ...
                <div>
                    学科： <a onClick={this.onSelectPaper.bind(this)} data-paperid='all' style={{ textDecoration: 'none', color: this.state.currentPaper.name === '全科' ? '#4489fc' : '#333', margin: '0 10px' }} href='javascript:;'>全科</a>
                    {
                        examInfo.papers.map((subjectObj, index) => {
                            return (
                                <a key={'papers-' + index} onClick={this.onSelectPaper.bind(this)} data-paperid={subjectObj.pid} href='javascript:;' key={'subjects-' + index} style={{ textDecoration: 'none', color: this.state.currentPaper.name === subjectObj.name ? '#4489fc' : '#333', margin: '0 10px' }}>{subjectObj.name}</a>
                            )
                        })
                    }
                </div>
...
class RankReport extends React.Component {
    ...
    getHeadSeq() {
        _.forEach(examInfo.papers, paperObj=> {
            _.forEach(['score', 'groupRank', 'classRank'], item => {
                this.headSeq.push(item + '_' + paperObj.pid)
            })
        })
    }
    ...
    render() {
        ...
        //todo 根据rankCache是否存在来显示loading图标;
        ...
        return (
            <div style={{ width: 1000, minHeight: 830, backgroundColor: '#fff', margin: '0 auto', marginTop: 30 }}>
                <div style={{ paddingLeft: 20, backgroundColor: '#fcfcfc', height: 75, lineHeight: '75px', textAlign: 'center' }}>
                    <a href='javascript:;' style={{ fontSize: 12, textDecoration: 'none', color: '#333', float: 'left' }}> {String.fromCharCode(60) } 返回</a>
...
export default RankReport;//connect(mapStateToProps, mapDispatchToProps)(RankReport);

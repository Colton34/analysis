    ...
    return (
        <div>
            ...
            <div className={commonClass['analysis-conclusion']}>
                ...
                <p>高分段学生密集的班级排名：{_.join(summaryInfo.high.map(item => {return item.className}), '、')}。其中{summaryInfo.high[0].className}高分段人数最多。</p>
                <p>高分段学生密集的班级排名：{_.join(summaryInfo.low.map(item => {return item.className}), '、')}。其中{summaryInfo.low[0].className}低分段人数最多，需要注意。</p>
                <div>学生学科成绩两极分化的班级：{summaryInfo.polar.length ? _.join(summaryInfo.polar, '、') : '无'}。</div>
            </div>
        </div>
    )
}

...
export default function SubjectScoreLevelDistribution({reportDS, currentClass}) {
...
// export default function SubjectScoreLevelDistribution({reportDS, currentClass}) {
//     var examPapersInfo = reportDS.examPapersInfo.toJS(), allStudentsPaperMap = reportDS.allStudentsPaperMap.toJS(), headers = reportDS.headers.toJS();
//     var tableDS = getTableDS(examPapersInfo, allStudentsPaperMap, headers, currentClass);
//     debugger;
// }

function getTableDS(examPapersInfo, allStudentsPaperMap, headers, currentClass, levelPcentages=[0, 60, 70, 85, 100]) {//Note: 这个等级是固定不变的
...
//segments依然是从小到大，但这里展示的时候是从大到小（高难度档次在前）
    ...
    _.each(subjectHeaders, (headerObj, index) => {
        ...
        var currentClassSubjectStudents = _.filter(allStudentsPaperMap[headerObj.id], (obj) => obj['class_name'] == currentClass);

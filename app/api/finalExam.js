...
//初始化数据：
        ...
        var headers = [];
        _.each(examPapersInfo, (paper, pid) => {//subjectWeight中科目的顺序即是权重，这里是找出当前考试有哪些科目，并添加pid的信息
        ...
        headers.unshift({subject: '总分', id: 'totalScore' }); //总分排在第一位

        var levels = makeDefaultLevles(examInfo, examStudentsInfo); //Done
        // console.log(levels);
...
//================================================================================================
        ...
        //思路：初始化数据好了（放在reducer中，这些数据都是最为school analysis的状态树上的属性数据），levels也作为属性数据
        //显示各个Module，把这些数据作为原料送过去，然后计算就在自己的Module中计算
